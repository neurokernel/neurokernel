...
class MorrisLecar: 
    def __init__(self, N, num_types, num_cart, neuron_start, dt, num_dendrite, V0, n, V1, V2, V3, V4, Tphi, offset, non_input_start):
        """
    ...
    def get_euler_kernel(self, neuron_start, V_1, V_2, V_3, V_4, Tphi, offset):
        ...
        mod = SourceModule(template.read() % {"type": dtype_to_ctype(dtype), "ntype": self.num_types, "nneu": self.update_block[0]}, options=["--ptxas-options=-v"])
    ...
    def get_euler_kernel1(self, neuron_start, V_1, V_2, V_3, V_4, Tphi, offset):
        ...
        dtype = self.dtype
        scalartype = dtype.type if dtype.__class__ is np.dtype else dtype
        mod = SourceModule(template.read() % {"type": dtype_to_ctype(dtype), "ntype": self.num_types}, options=["--ptxas-options=-v"])
        func = mod.get_function("hhn_euler_multiple")
    ...
    def get_input_func(self):
        ...
        mod = SourceModule(template.read() % {"num_neurons": self.num_neurons}, options = ["--ptxas-options=-v"])
