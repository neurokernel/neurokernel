...
def write_memory_to_file(A, filename, mode = 'w', title = 'test'):
    ...
    mode: 'w' to start a new file
          'a' to append, leading dimension of A must be the same as the
          existing file
...
def read_file(filename):
    """
    read a h5 file generated by write_memory_to_file or in Matlab using
    h5write.m returns a ndarray
    ...
    if n == 1:
        ...
        cl = r.dtype
        a = h5file.root.real.read() + (np.array(1j).astype(np.complex64)) * \
            h5file.root.imag.read()
