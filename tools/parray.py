...
# This is the parray class (pitched array) that serves as a complement to
# pycuda.GPUArray.
# The intention here is to automatically create 2D or 3D array with a pitched
# structure and varies operation on it.
...
class PitchArray(object):
    def __init__(self, shape, dtype, gpudata = None, pitch = None):
        ...
        pitch: if gpudata is specified and pitch is True, gpudata will be
               treated as if it was allocated by cudaMallocPitch with pitch
        ...
        .N: self.size if self is a vector, product of shape[1] and shape[2]
            otherwise
        ...
        if isinstance(other, PitchArray):
            ...
            if self.size:
                if self.M == 1:
                    ...
                    func.prepared_call(self._grid, result.gpudata, self.gpudata, other.gpudata, self.size)
                else:
                    func = pu.get_addarray_function(self.dtype, other.dtype, result.dtype, pitch = True)
                    func.prepared_call(self._grid, self.M, self.N, result.gpudata, result.ld, self.gpudata, self.ld, other.gpudata, other.ld)
            return result
        else:
            if other == 0:
                return self.copy()
                ...
                if self.size:
                    if self.M == 1:
                        ...
                        func.prepared_call(self._grid, result.gpudata, self.gpudata, other, self.size)
                    else:
                        func = pu.get_addscalar_function(self.dtype, pitch = True)
                        func.prepared_call(self._grid, self.M, self.N, result.gpudata, result.ld, self.gpudata, self.ld, other)
                return result
        ...
        if isinstance(other, PitchArray):
            ...
            if self.size:
                if self.M == 1:
                    ...
                    func.prepared_call(self._grid, result.gpudata, other.gpudata, self.gpudata, self.size)
                else:
                    func = pu.get_subarray_function(other.dtype, self.dtype, result.dtype, pitch = True)
                    func.prepared_call(self._grid, self.M, self.N, result.gpudata, result.ld, other.gpudata, other.ld, self.gpudata, self.ld)
            return result
        else:
        ...
        if isinstance(other, PitchArray):
            ...
            if other == 0:
                ...
                if self.size:
                    if self.M == 1:
                        func = pu.get_addscalar_function(self.dtype, pitch = False)
                        func.prepared_call(self._grid, self.gpudata, self.gpudata, other, self.size)
                    else:
                        func = pu.get_addscalar_function(self.dtype, pitch = True)
                        func.prepared_call(self._grid, self.M, self.N, self.gpudata, self.ld, self.gpudata, self.ld, other)
                return self
    ...
    def fill(self, value, stream = None):
        ...
        if self.size:
            if self.M == 1:
                ...
                func.prepared_call(self._grid, self.size, self.gpudata, value)
            else:
                func = pu.get_fill_function(self.dtype, pitch = True)

                func.prepared_call(self._grid, self.M, self.N, self.gpudata, self.ld, value)
    ...
    def real(self):
        ...
        if self.size:
            if self.M == 1:
                ...
                func.prepared_call(self._grid, result.gpudata, self.gpudata, self.size)
            else:
                func = pu.get_realimag_function(self.dtype, real = True, pitch = True)
                func.prepared_call(self._grid, self.M, self.N, result.gpudata, result.ld, self.gpudata, self.ld)
        return result
    ...
    def T(self):
        ...
        if self.size:
            ...
            func.prepared_call(self._grid, self.shape[0], self.shape[1], result.gpudata, result.ld, self.gpudata, self.ld)
