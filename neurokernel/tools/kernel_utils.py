...
def launch_kernel(func, block, grid, argins, timed = None, shared = None,
                  texrefs = [], prepared = False):
    ...
    for arg in range(Nargin):
        ...
        if a.__class__ == tuple or a.__class__ == list:
            ...
            if b.__class__ == garray.GPUArray:
                ...
                arg_call.append(np.intp(int(b.gpudata) + offset * \
                                        b.dtype.itemsize))

            elif b.__class__ == parray.PitchArray:
                arg_type.append(np.intp)
        ...
        elif a.__class__ == parray.PitchArray:
            arg_type.append(np.intp)
            arg_call.append(a.gpudata)
    ...
    if not prepared:
        func.prepare(arg_type, block, shared, texrefs)

    new_grid = (int(grid[0]), int(grid[1]))

    if timed is None:
        func.prepared_call(new_grid, *arg_call)

    else:
        a = func.prepared_timed_call(new_grid, *arg_call)
        if timed == 1:
            return a()
        else:
            time = a()
...
def func_compile(func_name, source, options = ["--ptxas-options=-v"],
                 keep = False, no_extern_c = False, arch = None, code = None,
                 cache_dir = None):
    from pycuda.compiler import SourceModule

    mod = SourceModule(source, options = options, keep = keep,
                       no_extern_c = no_extern_c, arch = arch, code = code,
                       cache_dir = cache_dir)
