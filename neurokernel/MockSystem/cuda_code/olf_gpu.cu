...
__global__ void gpu_run_dt(double dt, int neu_num, int syn_num,
		LeakyIAF *neuron, int *neu_syn_list, AlphaSyn *synapse,
		AlphaSynNL *syn_neu_list, int *I_ext_map, double *I_ex, int *spike_list) {
...
__global__ void gpu_run(int N, double dt, int neu_num, LeakyIAF *neuron,
		int *neu_syn_list, int syn_num, AlphaSyn *synapse,
		AlphaSynNL *syn_neu_list, int *spike_list, int *I_ext_map,
		int I_ext_num, int I_ext_len, double *I_ext) {
	...
	if (uid < neu_num)
		neu_thread_copy( neuron[uid], bh, ic, n_s_list, neu_syn_list, cid,
				I_ext_map[uid], eid);
	...
	if (uid < syn_num)
		syn_thread_copy( synapse[uid], num, tau_r, gmax, sign, s_n_list,
				syn_neu_list);
	...
	for (int i = 0; i < N; ++i) {
		...
		// Update Synapse Status
		if (uid < syn_num)
			update_syn_G( synapse[uid].g, num, g_old, g_new, gmax, tau_r, sign,
					s_n_list, dt_spk_list);

		// Update External Current( only for sensory neuron )
		I = 0.0;
		if (cid != -1 && i < I_ext_len)
			I = I_ext[eid];
...
__global__ void spk_rate(int N, float dt, float overlap, int neu_num,
		int *spk_list, double *rate) {
	const int tid = threadIdx.x + threadIdx.y * blockDim.x;
